"""
PDF Generator using WeasyPrint (Lightweight, no browser required)
Works on limited environments like Render free tier
"""
...
def generate_fleet_guide_pdf(conversation_id: int, db: Session) -> str:
    """
    Generate fleet guide PDF using WeasyPrint
    ...
    if conversation.recommended_ships:
        for ship_rec in conversation.recommended_ships:
            # Handle both formats: with ship_id or just ship data
            ship_id = ship_rec.get("ship_id") or ship_rec.get("id")

            if ship_id:
                # Look up full ship details from database
                ship = db.query(Ship).filter(Ship.id == ship_id).first()
                if ship:
                    ships_data.append({
                        "name": ship.name,
                        "manufacturer": ship.manufacturer.name if ship.manufacturer else ship_rec.get("manufacturer", "Unknown"),
                        "focus": ship.focus or ship_rec.get("focus", "Multi-Role"),
                        ...
                        "reason": ship_rec.get("recommendation_reason", ship_rec.get("reason", "Recommended based on your preferences")),
                ...
                # Use data from ship_rec directly (fallback for when ship not in DB)
                ships_data.append({
                    "name": ship_rec.get("name", "Unknown Ship"),
                    "manufacturer": ship_rec.get("manufacturer", "Unknown"),
                    "focus": ship_rec.get("focus", "Multi-Role"),
                    "type": "Multi-Role",
                    "price_usd": ship_rec.get("price_usd"),
                    "description": f"Recommended for {ship_rec.get('recommendation_reason', 'your needs')}",
                    "reason": ship_rec.get("recommendation_reason", "Recommended based on conversation"),
                    "crew_min": ship_rec.get("crew_min", 1),
                    "crew_max": ship_rec.get("crew_max", 1),
                    "cargo_capacity": ship_rec.get("cargo_capacity", 0),
                    "speed_scm": None,
                    "speed_max": None,
                })
    ...
    template = env.get_template("fleet_guide_weasyprint.html")
    ...
    print(f"✅ Generated fleet guide PDF: {pdf_path}")
    return str(pdf_path)


def generate_transcript_pdf(conversation_id: int, db: Session) -> str:
    """
    Generate transcript PDF using WeasyPrint
    ...
    template_dir = Path(__file__).parent.parent.parent / "templates"
    env = Environment(loader=FileSystemLoader(str(template_dir)))
    template = env.get_template("transcript_weasyprint.html")
    ...
    print(f"✅ Generated transcript PDF: {pdf_path}")
    return str(pdf_path)


def generate_both_pdfs(conversation_id: int, db: Session) -> Dict[str, str]:
    """
    Generate both PDFs (transcript and fleet guide) using WeasyPrint
    ...
    # Generate both PDFs
    transcript_path = generate_transcript_pdf(conversation_id, db)
    fleet_guide_path = generate_fleet_guide_pdf(conversation_id, db)
...
if __name__ == "__main__":
    ...
    try:
        ...
        conv_id = int(sys.argv[1]) if len(sys.argv) > 1 else 31
        print(f"Generating PDFs for conversation {conv_id}...")

        paths = generate_both_pdfs(conv_id, db)
        print(f"\n✅ SUCCESS!")
