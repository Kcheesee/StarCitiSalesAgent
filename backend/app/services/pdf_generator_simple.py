"""
Simple PDF Generator using xhtml2pdf (Pure Python - No System Dependencies)
Works everywhere: localhost, Render, any platform
"""
...
def _generate_fleet_analysis(ships: List[Dict], preferences: Dict) -> str:
    ...
    analysis += f"Recommended deployment strategy: Start with your most versatile vessel and expand operations as you gain experience and resources."
...
def generate_fleet_guide_pdf(conversation_id: int, db: Session) -> str:
    """
    Generate fleet guide PDF using xhtml2pdf (pure Python)
    """
    # Get conversation
    ...
    template = env.get_template("fleet_guide_simple.html")
...
def generate_transcript_pdf(conversation_id: int, db: Session) -> str:
    """
    Generate transcript PDF using xhtml2pdf (pure Python)
    """
    # Get conversation
    conversation = db.query(Conversation).filter(Conversation.id == conversation_id).first()
    if not conversation:
    ...
    template_dir = Path(__file__).parent.parent.parent / "templates"
    env = Environment(loader=FileSystemLoader(str(template_dir)))
    template = env.get_template("transcript_simple.html")
...
def generate_both_pdfs(conversation_id: int, db: Session) -> Dict[str, str]:
    """
    Generate both PDFs (transcript and fleet guide)
    """
    conversation = db.query(Conversation).filter(Conversation.id == conversation_id).first()
    if not conversation:
        raise ValueError(f"Conversation {conversation_id} not found")
